...
<div style="display: flex; justify-content: center; margin-left: -250px">
	<!-- customer_FAQ -->
    <div id="customer_FAQ" style="margin-left: -100px; margin-top: 100px; display: none">
	    ...
	    <div class="uk-width-1-2@s uk-width-2-5@m" style="margin-left: 350px; border-top: 1px solid #cccccc; width: 1024px;">
			...
			<c:choose>
				...
				<li class="uk-parent" >
					<a href="#" class="side_menu" style="font-family: 'BM JUA_TTF';">
						<div style="display: flex; justify-content: space-between;">
							<p style="font-family: 'BM JUA_TTF';">${ QNA.num }.</p>
					...
					<ul class="uk-nav-sub">
					  	<div style="padding-bottom: 20px; background-color: #fcfcfc; display: flex; flex-direction: column; justify-content: space-between;">
					  		<div style="display: flex; justify-content: space-between; align-items: center;">
					  			<div style="width: 900px">
									<pre id="content" name="content" style="outline: none; margin-top: 20px; padding: 0 12px; font-family: 'BM JUA_TTF';">${ QNA.content }</pre>
					  			</div>
								<div style="display: flex; justify-content: flex-end; align-items: center;">
									<p style="margin-top: 20px; font-family: 'BM JUA_TTF';">
									<c:if test="${ not empty sessionScope.u_id }" var="result1">
										<input type="hid-den" id="like_count" name="like_count" value="${ sessionScope.likeCheck }"/>
										<input type="hid-den" id="num" name="num" value="${ QNA.num }"/>
										<c:if test="${ sessionScope.likeCheck eq 1 }">
										...
										<c:if test="${ sessionScope.likeCheck eq 0 || sessionScope.likeCheck eq null }">
									...
									<c:if test="${ not result1 }">
										...
										<p style="margin-right: 20px">${ QNA.likecount }</p>
									</p>
								</div>
					  		</div>
						</div>
						<div style="margin-top: 50px; display: flex; justify-content: space-between;">
						<c:if test="${ not empty QNA.ofile }">
						<div style="display: flex; justify-content: flex-start;">
							<img src="../Uploads/${ QNA.sfile }" alt="" width="200px"/>
							<a href="./download?ofile=${ QNA.ofile }&sfile=${ QNA.sfile }&num=${ QNA.num }" style="padding-top: 150px">
								<img src="./image/download.png" alt="이미지" width="20px"/>
							</a>
						</div>
						</c:if>
						<c:if test="${ sessionScope.u_id eq QNA.id }" var="result">
						<form style="display: flex; justify-content: flex-end;">
						<div style="margin-top: 150px;">
							<input type="button" onclick="location.href='./edit?num=${ QNA.num }'" value="수정하기" style="border: 0; background: none; color: black; padding-right: 20px; width: 60px;" />
							<input type="button" onclick="location.href='./delete?num=${ QNA.num }'" value="삭제하기" style="border: 0; background: none; color: black; width: 60px;" />
						</div>
						</form>
						</c:if>
